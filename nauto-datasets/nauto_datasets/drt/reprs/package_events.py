...
def load_events(events_fname, s3_client, s3_bucket, s3_key, ts_fields=[]):
    ...
    obj = s3_client.get_object(Bucket=s3_bucket,
                               Key='{}/{}'.format(s3_key, events_fname))
...
def load_events_media_messages(videos_fname,
                               sensors_fname,
                               s3_client,
                               s3_bucket,
                               s3_key,
    ...
    for i, row in event_media.iterrows():
        try:
            ...
            if 'snapshot' in row.message_type:
                ...
                message_extension = ''
            fname = '{}-{}-{}{}'.format(row.device_id, row.message_id,
                                        row.message_type, message_extension)
...
def get_event_media_messages(event, video_messages, sensor_messages):
    """Get related event media messages from list of video and
    sensor messages."""
    event_media = (pd.concat(
        (sensor_messages.loc[sensor_messages.message_id.isin(
            event['sensors'])].copy(),
         video_messages.loc[video_messages.message_id.isin(
             event['videos'])].copy()),
        sort=False).drop_duplicates(
        subset=['device_id', 'message_id', 'message_type']))
    event_media = event_media.loc[event_media.device_id ==
                                  event['device_id']].reset_index(drop=True)
...
def get_event_media_check_df(event,
                             event_media,
    ...
    # Create dataframe of expected event media from event
    # (filter non-message_id values)
    event_media_check_dict = {
        'video-in-ts': [v for v in event['videos'] if len(v) == 16],
    ...
    event_media_check = event_media_check.merge(
        event_media.assign(available=True),
        how='left',
        on=['message_type', 'message_id'])
    event_media_check.available = event_media_check.available.fillna(False)

    # Compare sensor_duration and video_trans_duration to check if timestamps
    # are valid
    event_media_check.loc[:, ['sensor_duration', 'video_trans_duration'
                              ]] = event_media_check.loc[:, [
                                                                'sensor_duration', 'video_trans_duration'
                                                            ]].fillna(0).astype(float)
    event_media_check['valid_video_duration'] = (
            (event_media_check.sensor_duration -
             event_media_check.video_trans_duration).abs() <= dur_diff_tol)

    # Check expected media files were downloaded successfully
    if check_downloaded is True:
        flist = os.listdir(media_dir)
        event_media_check['downloaded'] = event_media_check.fname.apply(
            lambda x: x in flist)
...
def check_event_media_valid(event_media_check, check_downloaded=False):
    ...
    if event_media_check.loc[event_media_check.media_type ==
                             'video'].shape[0] == 0:
    ...
    if not event_media_check.loc[event_media_check.media_type ==
                                 'video'].is_sensor_time.all():
        return "failed_video_sensor_time"
    if not event_media_check.loc[event_media_check.media_type ==
                                 'video'].valid_video_duration.all():
...
def run_probe(filename, cmd='ffprobe'):
    ...
    """
    args = [
        cmd, '-show_format', '-show_streams', '-show_frames', '-of', 'json',
        filename
    ]
...
def get_frames(row, frames, directory, event_utc_basetime=0):
    ...
    frame_info['frame_num'] = frame_info.assign(
        n=1).sort_values('pkt_pts').groupby('media_type').n.cumsum() - 1
    frame_info['sensor_ts'] = (
            frame_info.pkt_pts_time.apply(lambda x: int(float(x) * 1e9)) +
...
def get_event_frames(event_media, directory, utc_basetime):
    ...
    frames = []
    (event_media.loc[event_media.media_type == 'video'].apply(
        get_frames, args=(frames, directory, utc_basetime), axis=1))
    frames = (pd.concat(frames, sort=False).sort_values(
        ['message_type', 'media_type', 'sensor_ts']).reset_index(drop=True))
    frames['frame_ts_diff'] = frames.groupby(['message_type', 'media_type'
                                              ]).sensor_ts.diff().div(1e6)
    return frames
...
def get_event_frame_stats(event_media, frames):
    ...
    frame_stats = (pd.DataFrame(vint_cts).join(vext_cts, rsuffix='_out').join(
        aint_cts, rsuffix='_audio').sort_index().rename(
        columns={
            'source': 'vid_in_frames',
            'source_out': 'vid_out_frames',
            'source_audio': 'audio_frames'
        }))

    frame_type_stats = frames.groupby(
        ['message_type',
         'media_type']).frame_ts_diff.describe(percentiles=[0.5])
    frame_type_stats['ave_frame_rate'] = frame_type_stats['count'].div(
        event_media.loc[event_media.media_type == 'video'].groupby(
            'message_type').video_trans_duration.sum())
...
def get_trims(frames, min_frame_ts, max_frame_ts):
    ...
    trim_df = (f_df.assign(f_start_offset=(f_df.sensor_ts - min_frame_ts) /
                                          1e9).assign(f_start=f_df.frame_num).assign(
        f_end=f_df.frame_num + 1).groupby('fpath').agg({
        'f_start':
            'first',
        'f_end':
            'last',
...
def get_component_trims(frames, preserve_frames=False):
    ...
    if preserve_frames is False:
        ...
        min_frame_ts = np.max(
            (vframes_int.sensor_ts.min(), vframes_ext.sensor_ts.min()))
        max_frame_ts = np.min(
            (vframes_int.sensor_ts.max(), vframes_ext.sensor_ts.max()))
    ...
    if len(audio_frames) > 0:
        ...
        audio_nb_samples = aframes.drop_duplicates(
            ['fpath'], keep='first').nb_samples.astype(int).values
...
def create_event_video(trims,
                       output_dir,
                       fname,
                       join_videos=True,
                       join_type='horizontal-int-ext',
                       extract_audio=False,
    ...
    try:
        if preserve_frames is False:
            ...
            pts_sign = '-'
            pts_offset_int, pts_offset_ext, pts_offset_audio = vext_trim[0][
                                                                   3], vint_trim[0][3], vext_trim[0][3]
        else:
            # Get offset to preserve all video frames
            pts_sign = '+'
            pts_offset_int, pts_offset_ext, pts_offset_audio = vint_trim[0][
                                                                   3], vext_trim[0][3], vint_trim[0][3]

        # Trim and concatenate videos
        vid_int = [
            ffmpeg.input(x[0])['v'].trim(start_frame=x[1],
                                         end_frame=x[2]).setpts('PTS-STARTPTS')
            for x in vint_trim
        ]
        joined_int = ffmpeg.concat(*vid_int).setpts('PTS{}{}/TB'.format(
            pts_sign, pts_offset_int))
        vid_ext = [
            ffmpeg.input(x[0]).trim(start_frame=x[1],
                                    end_frame=x[2]).setpts('PTS-STARTPTS')
            for x in vext_trim
        ]
        joined_ext = ffmpeg.concat(*vid_ext).setpts('PTS{}{}/TB'.format(
            pts_sign, pts_offset_ext))

        # Trim and concatenate audio if present
        if len(audio_trim) > 0:
            audio_int = [
                ffmpeg.input(x[0])['a'].filter('atrim',
                                               start_sample=x[1],
                                               end_sample=x[2]).filter(
                    'asetpts', 'PTS-STARTPTS')
                for x in audio_trim
            ]
            joined_audio = ffmpeg.concat(*audio_int, v=0, a=1).filter(
                'asetpts', 'PTS{}{}/TB'.format(pts_sign, pts_offset_audio))

        # Options for stitching - must be done in correct order to prevent
        # showing blank frames
        vids = [joined_int, joined_ext]

        # Create video
        os.makedirs(output_dir, exist_ok=True)
        if (join_videos is False) | (join_videos == 'both'):
            # Output separate videos, method depends on
            # if audio is present or not
            if len(audio_trim) > 0:
                args = (ffmpeg.output(
                    joined_int, joined_audio,
                    os.path.join(output_dir,
                                 '{}-internal.mp4'.format(fname))).get_args())
                p = subprocess.Popen(['ffmpeg'] + args[:-1] + ['-vsync', '2'] +
                                     [args[-1]] + ['-y'])
                p.wait()
            else:
                args = (joined_int.output(
                    os.path.join(output_dir,
                                 '{}-internal.mp4'.format(fname))).get_args())
                p = subprocess.Popen(['ffmpeg'] + args[:-1] + ['-vsync', '2'] +
                                     [args[-1]] + ['-y'])
                p.wait()
            args = (joined_ext.output(
                os.path.join(output_dir,
                             '{}-external.mp4'.format(fname))).get_args())
            p = subprocess.Popen(['ffmpeg'] + args[:-1] + ['-vsync', '2'] +
                                 [args[-1]] + ['-y'])
            p.wait()
        if (join_videos == 'internal'):
            # Output internal videos, method depends on if audio is present
            # or not
            if len(audio_trim) > 0:
                args = (ffmpeg.output(
                    joined_int, joined_audio,
        ...
        if (join_videos is True) | (join_videos == 'both'):
            ...
            if len(audio_trim) > 0:
                args = (ffmpeg.output(
                    ...
                    os.path.join(output_dir,
                                 '{}.mp4'.format(fname))).get_args())
                p = subprocess.Popen(['ffmpeg'] + args[:-1] + ['-vsync', '2'] +
                                     [args[-1]] + ['-y'])
                p.wait()
            else:
                args = (joined_video.output(
                    os.path.join(output_dir,
                                 '{}.mp4'.format(fname))).get_args())
                p = subprocess.Popen(['ffmpeg'] + args[:-1] + ['-vsync', '2'] +
                                     [args[-1]] + ['-y'])
        ...
        if extract_audio is True:
            if len(audio_trim) > 0:
                args = (joined_audio.output(
                    os.path.join(output_dir,
                                 '{}-audio.aac'.format(fname))).get_args())
...
def process_event_video(event,
    ...
    if event_media.loc[event_media.media_type == 'video'].shape[0] == 0:
        ...
        frames = get_event_frames(event_media, media_dir,
                                  event['utc_basetime'])
        trims, frame_timestamps, frame_ts_lims = get_component_trims(
            frames, preserve_frames=preserve_frames)
        status['create_video'] = create_event_video(
            trims,
            output_dir,
            media_fname,
...
def get_recording(files, convert_utc=True):
    ...
    proto_msgs = [
        protobuf.parse_message_from_gzipped_file(sensor_pb2.Recording,
                                                 file_path)
        for file_path in files
    ]

    com_recordings = sensors.CombinedRecording.from_recordings(
        [sensors.Recording.from_pb(r_pb) for r_pb in proto_msgs])
...
def orient_data(data, phi_xz):
    ...
    """
    trans_mtx = np.array([[np.cos(phi_xz), 0, -np.sin(phi_xz)], [0, 1, 0],
                          [np.sin(phi_xz), 0,
                           np.cos(phi_xz)]])
...
def get_oriented_imu(acc_in, gyro_in, orientation, method='global'):
    ...
    if method == 'global':
        ...
        gravity = np.matrix([0, 0, -np.array(acc_rot.mean(axis=1))[2][0]
                             ]).astype(np.float64).transpose()
        acc.loc[:, axes] = (acc_rot + gravity).T
        gyro.loc[:, axes] = orient_data(
            np.matrix(gyro.loc[:, axes].values).T, phi_xz).T
    ...
    try:
        ...
        if len(orientation) == 0:
            ...
            if len(orientation) == 0:
                ...
                orientation = orientation.loc[orientation.converged].iloc[-1][[
                    'pitch', 'roll'
                ]]
        else:
...
def load_sensor_fields(com_rec, fields=['acc', 'gyro', 'gps']):
    ...
    for field in fields:
        if field not in com_rec._fields:
            continue
        sensor_data[field] = (getattr(com_rec, field).stream._to_df().drop(
            'system_ms', axis=1).rename(columns={
            'sensor_ns': 'timestamp_utc_ns'
        }).drop_duplicates(subset=['timestamp_utc_ns']).reset_index(
            drop=True))
    if ('applied_orientation' in fields) & (len(
            sensor_data['applied_orientation']) == 0):
...
def load_sensor_data(event_media, media_dir, fields=['acc', 'gyro', 'gps']):
    ...
    flist = event_media.loc[event_media.media_type == 'sensor'].sort_values(
        'message_ts').fname.values
...
def trim_sensors_to_video(sensor_data,
                          video_ts_lims,
                          ignore_fields=['applied_orientation']):
    ...
    for k in sensor_data.keys():
        if (type(sensor_data[k]) == pd.DataFrame) & (k not in ignore_fields):
            sensor_data[k] = (sensor_data[k].loc[
                (sensor_data[k].timestamp_utc_ns >= sensor_start)
                & (sensor_data[k].timestamp_utc_ns <= sensor_end)].reset_index(
                drop=True))
    return sensor_data
...
def match_cnn_output_to_frames(cnn_df,
                               frames_ts,
                               timestamp_field='timestamp_utc_ns',
                               tolerance=230):
    """Attempt to match frame scores to an actual frame timestamp."""
    vid_int_idx = pd.Index(frames_ts)
    cnn_df[timestamp_field] = cnn_df[timestamp_field].apply(
        find_nearest_value, idx=vid_int_idx, tolerance=tolerance)
    return cnn_df
...
def moving_average(data, n_samples=20):
    ...
    """
    return np.hstack(
        (np.cumsum(data[:n_samples - 1]) / np.arange(1, n_samples),
...
def get_imu_filt(imu,
    ...
    if interpolate_method is not None:
        imu_filt = (imu_filt.set_index('timestamp_utc_ns').interpolate(
            method=interpolate_method, limit=interpolate_limit).reset_index())

    if maneuver_filt is not None:
        for a in ['x', 'y', 'z']:
            imu_filt['acc_{}'.format(a)] = moving_average(
                imu_filt['acc_{}'.format(a)].values,
                int(maneuver_filt / 1000 * 200))
            imu_filt['gyro_{}'.format(a)] = moving_average(
                imu_filt['gyro_{}'.format(a)].values,
                int(maneuver_filt / 1000 * 200))
...
def get_combined_imu_df(acc, gyro, timestamp_field='timestamp_utc_ns'):
    ...
    cols = [
        timestamp_field, 'x_acc', 'y_acc', 'z_acc', 'x_gyro', 'y_gyro',
        'z_gyro'
    ]
    imu_df = (acc.join(gyro.set_index(timestamp_field),
                       ...
                       how='outer',
                       lsuffix='_acc',
                       rsuffix='_gyro')[cols].sort_values(timestamp_field))
    cols = [
        timestamp_field, 'acc_x', 'acc_y', 'acc_z', 'gyro_x', 'gyro_y',
        'gyro_z'
    ]
    imu_df.columns = cols
    return imu_df
...
def plot_event(sensor_data,
    ...
    try:
        ...
        if len(sensor_data['gps']) > 0:
            to_plot = (sensor_data['gps'].set_index(
                pd.to_datetime(sensor_data['gps'][timestamp_field],
                               unit=timestamp_units)))
            ax_id = 'gps'
            ax_num += 1
            ax_list.append({'ax_num': ax_num, 'id': ax_id, 'name': 'GPS'})
            chart_data.append(
                go.Scatter(x=to_plot.index,
                           y=to_plot['speed'],
        ...
        # Plot IMU
        imu_rot = get_combined_imu_df(sensor_data['oriented_acc'],
                                      sensor_data['oriented_gyro'])
        if len(imu_rot) > 0:
            ...
            if len(imu_filt) > 0:
                to_plot = imu_filt.set_index(
                    pd.to_datetime(imu_filt[timestamp_field],
                                   unit=timestamp_units))
                # Plot Gyroscope
                ax_id = 'gyro'
                ax_num += 1
        ...
        if len(sensor_data['dist_multilabel']) > 0:
            to_plot = (sensor_data['dist_multilabel'].set_index(
                pd.to_datetime(sensor_data['dist_multilabel'][timestamp_field],
                               unit=timestamp_units)))
            ax_id = 'dist'
            ax_num += 1
            ax_list.append({
        ...
        if len(sensor_data['tailgating']) > 0:
            to_plot = (sensor_data['tailgating'].set_index(
                pd.to_datetime(sensor_data['tailgating'][timestamp_field],
                               unit=timestamp_units)))
            # Fill non-detected front vehicle frames with NaNs
            to_plot.loc[to_plot.front_box_index ==
                        -1, 'distance_estimate'] = None
            ax_id = 'tail'
            ax_num += 1
            ax_list.append({
                'ax_num': ax_num,
...
def get_event_metadata(event, metadata_cols, created_by_email, rights):
    ...
    metadata = {
        'created_at': str(pd.datetime.utcnow()),
...
def get_event_data_dict(sensor_data=None):
    ...
    if 'bounding_boxes_external' in data.keys():
        for i, box in enumerate(
                data['bounding_boxes_external']['bounding_box']):
...
def process_sensor_data(
        ...
        media_dir,
        fields=[
            'gps', 'acc', 'gyro', 'oriented_acc', 'oriented_gyro',
            'applied_orientation', 'dist_multilabel', 'tailgating',
            'bounding_boxes_external'
        ],
        trim_to_video=False,
        frame_ts_lims=None,
        frame_timestamps=None,
        match_cnn_to_frames=True,
        cnn_field_params={
            'dist_multilabel': 230 * 1e6,
            'tailgating': 100 * 1e6,
            'bounding_boxes_external': 100 * 1e6
        },
        message_params_field=None,
        trim_ignore_fields=['applied_orientation']):
    ...
    if event_media.loc[event_media.media_type == 'sensor'].shape[0] == 0:
        ...
        if (trim_to_video is True) & (frame_ts_lims is not None):
            sensor_data = trim_sensors_to_video(
                sensor_data, frame_ts_lims, ignore_fields=trim_ignore_fields)

        if frame_timestamps is not None:
            # Get video and audio frame timestamps
            video_frames_ts = {
                'internal_timestamp_utc_ns': list(map(int,
                                                      frame_timestamps[0])),
                'external_timestamp_utc_ns': list(map(int,
                                                      frame_timestamps[1]))
            }
            audio_frames_ts = {
                'internal_timestamp_utc_ns': list(map(int,
            ...
            if match_cnn_to_frames is True:
                for field in cnn_field_params.keys():
                    sensor_data[field] = match_cnn_output_to_frames(
                        sensor_data[field],
        ...
        if message_params_field is not None:
            sensor_data['event_params'] = json.loads(
                event[message_params_field])

    return sensor_data


def handle_event_media_failure_cases(event_media, failure_status,
                                     preserve_frames):
    ...
    try:
        if failure_status == 'failed_video_sensor_time':
            ...
            """
            This bug should only affect a single inside or outside video in an
            event. Until we have a better method, we naively repair these by
            assuming sensor_end and video_trans_duration are correct and using
            this to fix sensor_start.
            """
            rows_to_fix = event_media.loc[
                (event_media.media_type == 'video')
                & ~event_media.valid_video_duration].index
            event_media.loc[rows_to_fix, 'sensor_start'] = event_media.loc[
                                                               rows_to_fix].sensor_end - (
                                                                   event_media.loc[rows_to_fix].video_trans_duration *
                                                                   1e9).astype(int)
            event_media.loc[rows_to_fix, 'sensor_duration'] = (
                    event_media.loc[rows_to_fix].sensor_end -
                    event_media.loc[rows_to_fix].sensor_start).div(1e9)
            failure_handling_status = 'success'
        elif failure_status in ('failed_media_downloaded',
                                'failed_media_available'):
            preserve_frames = True
            # failed_videos = (event_media.loc[not event_media.downloaded]
            # .message_id
            # .unique())
...
def process_event_media(
        ...
        preserve_frames=False,
        handle_failure_cases=[],
        join_videos=True,
        join_type='horizontal-ext-int',
        extract_audio=False,
        create_sensor_json=False,
        plot_sensor_data=False,
        message_params_field='event_message_params',
        metadata_cols=[
            'fleet_id', 'device_id', 'message_id', 'message_type', 'message_ts'
        ],
    ...
    """Check all event media. Includes option to download media for
    processing."""
    status = {}
    ...
    event_media = get_event_media_messages(event, video_messages,
                                           sensor_messages)
    ...
    if dry_run is False:
        event_media = download_event_media(event_media,
                                           s3_client,
                                           media_dir=media_dir,
                                           overwrite=overwrite_media)
    download_media = not dry_run

    event_media = get_event_media_check_df(event,
                                           event_media,
                                           check_downloaded=download_media,
                                           media_dir=media_dir)
    status['check_event_media'] = check_event_media_valid(
        event_media, check_downloaded=download_media)

    if (dry_run is True) | (status['check_event_media'] not in ['success'] +
                            handle_failure_cases):
        frames, fig, event_json = None, None, None
    else:
        # Handle failure cases
        if (status['check_event_media'] in handle_failure_cases) | (
                handle_failure_cases == 'all'):
            resp = handle_event_media_failure_cases(
                event_media, status['check_event_media'], preserve_frames)
            status[
                'handle_check_event_media_failure'], preserve_frames, event_media = resp

        # Process video
        video_result = process_event_video(event, event_media, status,
                                           media_fname, media_dir, output_dir,
                                           join_videos, join_type,
                                           preserve_frames, extract_audio)
        ...
        if sensor_data is not None:
            ...
            if create_sensor_json is True:
                ...
                status['create_sensor_json'] = save_event_json(
                    event_json, output_dir, media_fname)

            # Create HTML sensor data plot
            if plot_sensor_data is True:
                status['create_sensor_plot'], fig = plot_event(
                    sensor_data,
                    title=media_fname,
                    height=None,
                    output_dir=output_dir,
                    output_filename=media_fname)
        else:
            if create_sensor_json is True:
        ...
        if (s3_output is True) & (s3_output_dir is not None):
            try:
                for f in os.listdir(output_dir):
                    if media_fname in f:
                        s3_client.upload_file(os.path.join(output_dir, f),
                                              s3_bucket,
                                              os.path.join(s3_output_dir, f))
